...
import org.forome.database.exception.KeyCorruptedException;
...
public class HashIndexKey extends IndexKey {
    ...
    public static HashIndexKey unpack(final byte[] src) {
        ...
        byte[] attendant = new byte[BaseIndex.ATTENDANT_BYTE_SIZE];
    ...
    public static long unpackFirstIndexedValue(final byte[] src) {
        return TypeConvert.unpackLong(src, BaseIndex.ATTENDANT_BYTE_SIZE);
    }
    ...
    private static int readLongCount(final byte[] src) {
        final int fieldsByteSize = src.length - BaseIndex.ATTENDANT_BYTE_SIZE;
